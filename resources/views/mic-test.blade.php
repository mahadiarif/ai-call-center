...
<div class="container py-5" style="max-width: 700px;">
    ...
    <div class="chat-container mb-4 shadow-sm">
        ...
        <select id="ivrKeySelect" class="form-select w-75 mx-auto mb-3 text-center shadow-sm border-primary fw-bold">
            @forelse($ivrServices as $ivr)
                <option value="{{ $ivr->id }}">
                    @if($ivr->key_press == 1) 🔴 @else 💰 @endif 
                    Button {{ $ivr->key_press }}: {{ $ivr->service_name }} 
                    @if($ivr->voice_gender == 'Charon') (Male - {{ $ivr->voice_speed }}) @else (Female - {{ $ivr->voice_speed }}) @endif
                </option>
            @empty
